...
var app = {
  ...
  admin: function(roomId, username){
      ...
      socket.on('connect', function () {
        ...
        socket.on('updateUsersList', function(users, clear) {
          ...
          if(users.error != null){
            ...
            $(".mute.btn").on('click',(el)=>{
              socket.emit('toggleBlockUser',roomId , el.target.getAttribute('attr-user-id'));
            })
          }
        });
        ...
        for(var user of users) {
          ...
          html += `<li class="clearfix" id="user-${user._id}">
                     ...
                     <div class="about">
                        ...
                        <div class="mute btn" attr-user-id='${user.username}' >${user.mute?'unmute':'mute'}</div>
                     </div></li>`;
        }
